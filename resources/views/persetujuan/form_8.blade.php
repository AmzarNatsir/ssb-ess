<div class="modal-header bg-primary text-white border-0">
    <h5 class="modal-title fw-bold" id="exampleModalCenteredScrollableTitle">
        <i class="ti ti-plane-tilt me-2"></i>Form Persetujuan Pengajuan Perjalanan Dinas
    </h5>
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        @if(!empty($profil->get_profil->photo))
                            <img src="{{ url(Storage::url('hrd/photo/'.$profil->get_profil->photo)) }}" class="rounded-circle me-3" alt="avatar" style="width: 70px; height: 70px; object-fit: cover;">
                        @else
                    ...
                    <table class="table table-sm table-borderless mb-0">
                        <tr>
                            ...
                            <td class="fw-semibold">{{ date_format(date_create($profil->tgl_perdis), 'd-m-Y') }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Maksud / Tujuan</td>
                            <td class="fw-semibold">{{ $profil->maksud_tujuan }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Tanggal Berangkat</td>
                            <td class="fw-semibold text-primary">{{ date('d-m-Y', strtotime($profil->tgl_berangkat)) }} s/d {{ date('d-m-Y', strtotime($profil->tgl_kembali)) }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Tujuan</td>
                            <td class="fw-semibold">{{ $profil->tujuan }}</td>
                        </tr>
                        <tr>
