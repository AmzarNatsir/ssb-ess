<div class="modal-header bg-primary text-white border-0">
    <h5 class="modal-title fw-bold" id="exampleModalCenteredScrollableTitle">
        <i class="ti ti-calendar-event me-2"></i>Form Persetujuan Pengajuan Perubahan Masa Cuti
    </h5>
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        @if(!empty($profil->get_cuti_origin->profil_karyawan->photo))
                            <img src="{{ url(Storage::url('hrd/photo/'.$profil->get_cuti_origin->profil_karyawan->photo)) }}" class="rounded-circle me-3" alt="avatar" style="width: 70px; height: 70px; object-fit: cover;">
                        @else
                        ...
                        @endif
                        <div>
                            <div class="text-muted small">{{ $profil->get_cuti_origin->profil_karyawan->nik }}</div>
                            <div class="fw-bold">{{ $profil->get_cuti_origin->profil_karyawan->nm_lengkap }}</div>
                            <div class="text-muted small">{{ $profil->get_cuti_origin->profil_karyawan->get_jabatan->nm_jabatan }} | {{ $profil->get_cuti_origin->profil_karyawan->get_departemen->nm_dept }}</div>
                        </div>
                    </div>

                    <h6 class="fw-bold text-primary mb-3">
                        <i class="ti ti-edit me-2"></i>Data Pengajuan Perubahan
                    </h6>
                    <table class="table table-sm table-borderless mb-3">
                        <tr>
                            <td class="text-muted" style="width:40%">Tanggal Pengajuan</td>
                            <td class="fw-semibold">{{ date('d-m-Y', strtotime($profil->created_at)) }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Alasan Pengajuan</td>
                            <td class="fw-semibold">{{ $profil->alasan_perubahan }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Perubahan Masa Cuti</td>
                            <td class="fw-semibold text-primary">{{ date_format(date_create($profil->tgl_awal_origin), 'd-m-Y') }} s/d {{ date_format(date_create($profil->tgl_akhir_edit), 'd-m-Y') }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Jumlah Hari</td>
                            <td class="fw-semibold text-primary">{{ $profil->jumlah_hari_edit }} hari</td>
                        </tr>
                    </table>

                    <h6 class="fw-bold text-secondary mb-3">
                        <i class="ti ti-history me-2"></i>Data Pengajuan Awal
                    </h6>
                    <table class="table table-sm table-borderless mb-0">
                        <tr>
                            <td class="text-muted" style="width:40%">Rencana Cuti</td>
                            <td class="fw-semibold">{{ date_format(date_create($profil->tgl_awal_origin), 'd-m-Y') }} s/d {{ date_format(date_create($profil->tgl_akhir_origin), 'd-m-Y') }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Jumlah Hari</td>
                            <td class="fw-semibold">{{ $profil->jumlah_hari_origin }} hari</td>
                        </tr>
                    </table>
                </div>
            </div>
