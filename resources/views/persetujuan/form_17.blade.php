<div class="modal-header bg-primary text-white border-0">
    <h5 class="modal-title fw-bold" id="exampleModalCenteredScrollableTitle">
        <i class="ti ti-shield-off me-2"></i>Form Persetujuan Pengajuan Penonaktifan Surat Peringatan
    </h5>
            ...
            <div class="card bg-light border-0 h-100">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        @if(!empty($profil->getSP->profil_karyawan->photo))
                            <a href="{{ url(Storage::url('hrd/photo/'.$profil->getSP->profil_karyawan->photo)) }}" data-fancybox data-caption="avatar">
                            <img src="{{ url(Storage::url('hrd/photo/'.$profil->getSP->profil_karyawan->photo)) }}" class="rounded-circle me-3" alt="avatar" style="width: 70px; height: 70px; object-fit: cover;"></a>
                        @else
                        ...
                        @endif
                        <div>
                            <div class="text-muted small">{{ $profil->getSP->profil_karyawan->nik }}</div>
                            <div class="fw-bold">{{ $profil->getSP->profil_karyawan->nm_lengkap }}</div>
                            <div class="text-muted small">{{ $profil->getSP->profil_karyawan->get_jabatan->nm_jabatan }} | {{ $profil->getSP->profil_karyawan->get_departemen->nm_dept }}</div>
                        </div>
                    </div>
                    ...
                    <table class="table table-sm table-borderless mb-0">
                        ...
                        <tr>
                            ...
                            <td class="fw-semibold">{{ $profil->alasan_non_aktif }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Masa Berlaku SP</td>
                            <td class="fw-semibold">{{ date('d-m-Y', strtotime($profil->getSP->profil_karyawan->sp_mulai_active)) }} s/d {{ date('d-m-Y', strtotime($profil->getSP->profil_karyawan->sp_akhir_active)) }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Uraian Pelanggaran</td>
                            <td class="fw-semibold">{{ $profil->getSP->uraian_pelanggaran }}</td>
                        </tr>
                    </table>
                </div>
            </div>
