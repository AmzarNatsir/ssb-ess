<div class="modal-header bg-primary text-white border-0">
    <h5 class="modal-title fw-bold" id="exampleModalCenteredScrollableTitle">
        <i class="ti ti-mail-forward me-2"></i>Form Persetujuan Pengajuan Penerbitan Surat Teguran
    </h5>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-sm table-borderless mb-3">
                        <tr>
                            ...
                            <td class="fw-semibold">{{ $profil->get_karyawan->nik }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Nama Karyawan</td>
                            <td class="fw-semibold">{{ $profil->get_karyawan->nm_lengkap }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Jabatan</td>
                            <td class="fw-semibold">{{ $profil->get_karyawan->get_jabatan->nm_jabatan }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Departemen</td>
                            <td class="fw-semibold">{{ $profil->get_karyawan->get_departemen->nm_dept }}</td>
                        </tr>
                    </table>
                    ...
                    <table class="table table-sm table-borderless mb-0">
                        <tr>
                            ...
                            <td class="fw-semibold">{{ date_format(date_create($profil->tanggal_pengajuan), 'd-m-Y') }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Tanggal Kejadian</td>
                            <td class="fw-semibold">{{ date('d-m-Y', strtotime($profil->tanggal_kejadian)) }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Waktu Kejadian</td>
                            <td class="fw-semibold">{{ date('H:i', strtotime($profil->waktu_kejadian)) }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Tempat Kejadian</td>
                            <td class="fw-semibold">{{ $profil->tempat_kejadian }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Jenis Pelanggaran</td>
                            <td class="fw-semibold text-danger">{{ $profil->get_jenis_pelanggaran->jenis_pelanggaran }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Akibat Pelanggaran</td>
                            <td class="fw-semibold">{{ $profil->akibat }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Tindakan Perbaikan</td>
                            <td class="fw-semibold">{{ $profil->tindakan }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Rekomendasi Atasan</td>
                            <td class="fw-semibold">{{ $profil->rekomendasi }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Komentar Pelanggar</td>
                            <td class="fw-semibold">{{ $profil->komentar_pelanggar }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Diajukan Oleh</td>
                            <td class="fw-semibold">{{ $profil->get_diajukan_oleh->nm_lengkap }} - {{ $profil->get_diajukan_oleh->get_jabatan->nm_jabatan }}</td>
                        </tr>
                    </table>
                </div>
            </div>
